...
//
// Bootup the masternode, look for a 2000 AOP input and register on the network
//
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode(bool fSelectUnlocked)
{
    ...
    {
        if(out.tx->vout[out.i].nValue == GetMNCollateral()*COIN) { //exactly 5,000 AOP
...
vector<COutput> CActiveMasternode::SelectCoinsMasternodeForPubKey(std::string collateralAddress)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        if(out.tx->vout[out.i].scriptPubKey == scriptPubKey && out.tx->vout[out.i].nValue == GetMNCollateral()*COIN) { //exactly 5,000 AOP
            filteredCoins.push_back(out);
        }
    }
    return filteredCoins;
