    ...
    address(addr)
{
    ...
    if (label.trimmed().isEmpty()) {
        setWindowTitle(QString("AveroPay - Generate QR Code"));
    } else {
        setWindowTitle(QString("AveroPay - Generate QR Code for %1").arg(label));
    }
...
{
    QString ret = QString("AveroPay:%1").arg(address);
    int paramCount = 0;
