...
std::vector<char> imageContents((std::istreambuf_iterator<char>(imageFile)),
    ...
    CAmount nAmount = 0.1 * COIN; // 0.1 AOP
    ...
    {
      ui->txLineEdit->setText("ERROR: You need at least a 0.1 AOP balance to send proof of data.");
    }
    else
    {
      std::string sNarr;
...
std::vector<char> imageContents((std::istreambuf_iterator<char>(imageFile)),
    ...
    std::string bexp = "https://AveroPayexplorer.org/address/";
    //open url
...
{
  //go to block explorer
    std::string bexp = "https://AveroPayexplorer.org/tx/";
    //open url
