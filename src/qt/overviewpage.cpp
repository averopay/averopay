...
const QString BaseURL = "https://averopay.com/aopusd.php";
const QString BaseURL2 = "https://averopay.com/aopbtc.php";
double AveroPayx;
double AOPbtcx;
    ...
    filter(0)
{
    ...
    ui->labelWalletStatus->setText("(" + tr("Out of Sync Please Wait!") + ")");
    ui->labelTransactionsStatus->setText("(" + tr("Out of Sync Please Wait!") + ")");
...
if (what == BaseURL) // AveroPay Price
{

    // QNetworkReply is a QIODevice. So we read from it just like it was a file
    QString AveroPay = finished->readAll();
    AveroPayx = (AveroPay.toDouble());
    AveroPay = QString::number(AveroPayx, 'f', 2);

	dollarg = AveroPay;
}
if (what == BaseURL2) // AveroPay BTC Price
{

    // QNetworkReply is a QIODevice. So we read from it just like it was a file
    QString AOPbtc = finished->readAll();
    AOPbtcx = (AOPbtc.toDouble());
    AOPbtc = QString::number(AOPbtcx, 'f', 8);
